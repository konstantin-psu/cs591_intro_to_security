...
(function($)
{
    $.fn.lightBox = function(settings)
    {
        ...
        function _start(objClicked, jQueryMatchedObj)
        {
            ...
            {
                // Single image
                settings.imageArray.push(new Array(objClicked.getAttribute('href').replace(window.location.protocol + '//' + window.location.hostname, ''), objClicked.getAttribute('title')));
            }
            else
            {
                // Image set
                ...
                {
                    settings.imageArray.push(new Array(jQueryMatchedObj[i].getAttribute('href').replace(window.location.protocol + '//' + window.location.hostname, ''), jQueryMatchedObj[i].getAttribute('title')));
                }
            }
            while (settings.imageArray[settings.activeImage][0] != objClicked.getAttribute('href').replace(window.location.protocol + '//' + window.location.hostname, ''))
            {
        ...
        function _set_image_to_view()
        {
            ...
            if (settings.imageArray[settings.activeImage][0].indexOf('youtube') == -1)
            {
                // Preload image
            ...
            {
                // Display video
                $('#lightbox-image').replaceWith('<div id="lightbox-image" style="width: 640px; height: 360px; display: none;"><iframe id="ytplayer" type="text/html" width="640" height="360" src="' + settings.imageArray[settings.activeImage][0] + '?autohide=1&autoplay=1&modestbranding=1&origin=' + window.location.protocol + '//' + window.location.hostname + '&rel=0&showinfo=0" frameborder="0"></div>');
                $('#lightbox-nav').hide();
                _resize_container_image_box(640, 360);
            }
        };
        function _resize_container_image_box(intImageWidth, intImageHeight)
        {
            ...
            // Create effect
            $('#lightbox-container-image-box' + settings.theme).animate({ width: intWidth, height: intHeight }, settings.containerResizeSpeed, function() { _show_image(); });
        ...
        function _show_image()
        {
            ...
            {
                _show_image_data();
                if (settings.imageArray[settings.activeImage][0].indexOf('youtube') == -1) _set_navigation();
            });
        ...
        function _show_image_data()
        {
            ...
            $('#lightbox-container-image-data-box' + settings.theme).slideDown(400);
        }
