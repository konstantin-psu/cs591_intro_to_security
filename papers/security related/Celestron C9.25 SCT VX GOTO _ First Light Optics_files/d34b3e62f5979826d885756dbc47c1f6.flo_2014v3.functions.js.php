    ...
    jQuery.post(actionurl, data, function(response)
    {
        if (response.substr(0, 4) != 'http')
        {
            ...
            if (jQuery('.block_added').length || jQuery('.block_basket').length || jQuery('.block_viewed').length || jQuery('.block_welcome').length)
            {
                ...
                jQuery('#center div.isblock:not(:last):not(.hideblock):not(#shoptalk_wrap)').append('<div class="cb_divider"></div>');
...
function capsLock(e)
{
    ...
    {
        alertDialog('dialog-alert', 0.4, 'Caps Lock is on, passwords are case-sensitive');
        warningGiven = 1;
    }
}
...
function changeCard(obj)
{
    ...
    else
    {
        ...
        hideRow('issuerow1');
        if (document.getElementById('issuerow2')) hideRow('issuerow2');
        form.StartDateMonth.value = '';
        form.StartDateYear.value = '';
        form.IssueNumber.value = '';
    }
}

function checkCard(required, terms)
{
    var message = '';
    var form = document.getElementById('payment');
    form.CardNumber.value = form.CardNumber.value.replace(/[^0-9]/g, '');
    form.IssueNumber.value = form.IssueNumber.value.replace(/[^0-9]/g, '');
    form.CV2.value = form.CV2.value.replace(/[^0-9]/g, '');
    if (!form.CardName.value || !form.CardNumber.value || !form.ExpiryDateMonth.value || !form.ExpiryDateYear.value || !form.CV2.value) message = required;
    if (!message && document.getElementById('accept'))
    {
        if (!document.getElementById('accept').checked) message = terms;
    }
    if (message)
    {
...
function tablet()
{
    if ((navigator.userAgent.match(/ipad/i)) || ((navigator.userAgent.match(/touch/i) || navigator.userAgent.match(/android/i)) && (!mobile()))) return true; else return false;
}
...
if (!aconsole) jQuery(document).ready(function()
{
    ...
    if (typeof autocomp != 'undefined')
    {
        ...
        jQuery('input[name=for]').autocomplete({ source: function(request, response) { jQuery.ajax({ url: '/includes/autocomplete.php', dataType: 'json', data: { term: request.term, langid: aclaid, currid: accuid, userid: acusid }, success: function(data) { response(data); } }); }, open: function(event, ui) { var field = this; jQuery('.ui-autocomplete').each(function() { var spanwidth = 0; jQuery(this).find('span').each(function() { if (jQuery(this).outerWidth() > spanwidth) spanwidth = jQuery(this).outerWidth(); }); var imgwidth = 0; if (jQuery(this).find('img').length) imgwidth = 30; var totalwidth = spanwidth + imgwidth + 10; var maxwidth = jQuery(window).width() - jQuery(this).offset().left; if (totalwidth <= (jQuery(field).outerWidth())) { jQuery(this).css('width', jQuery(field).outerWidth()); } else if (totalwidth <= maxwidth) { jQuery(this).css('width', totalwidth); } else { jQuery(this).css('width', totalwidth).position({ of: jQuery(field), my: 'right top', at: 'right bottom' }); } }); }, focus: function(event, ui) { if (ui.item.value != ui.item.label.replace('<span>', '').replace('</span>', '')) return false; }, select: function(event, ui) { if (ui.item.value == 'ALLRESULTS') { jQuery(this).val(jQuery(this).val() + '...'); jQuery(this).closest('form').submit(); return false; } else if (ui.item.value != ui.item.label.replace('<span>', '').replace('</span>', '')) { window.location.href = '/products/' + ui.item.value + '.html?search=1'; return false; } else if (ui.item.value) { jQuery(this).val(ui.item.value); jQuery(this).closest('form').submit(); return false; } }, delay: 200, minLength: 2 });
...
function redraw(rtype)
{
    ...
    if (typeof abcurve != 'undefined')
    {
        ...
        if (jQuery.browser.msie && ((jQuery.browser.version < 8 && !mode) || mode < 8)) jQuery('form[name=details] .alternate, #shoptalk > table.tables').uncorner().corner(abcsize);
        jQuery('.basket, .alternate, .alternateb, .alternatet, #shoptalk > table.tables').not(':has(.jquery-corner)').each(function()
        {
...
function resize(rtype)
{
    ...
    if (!jQuery.browser.msie || ((jQuery.browser.version >= 8 && !mode) || mode >= 8)) jQuery('.gridp').each(function()
    {
        ...
        {
            scale = resizerow(rtype, scale, this);
        });
    });
}

function resizerow(rtype, scale, row)
{
    var prti = 0;
    ...
    if ((rtype) && (scale != 0))
    {
        if (scale < 0)
        {
            var first = jQuery(row).children('td:first').find('.prim img');
            if (rtype == 1)
            {
                if ((jQuery(first).length) && (parseInt(jQuery(first).css('max-width'), 10) > parseInt(jQuery(first).outerWidth(), 10))) scale = 1; else scale = 0;
            }
            else
            {
                if (jQuery(first).length) scale = 1; else $scale = 0;
            }
        }
        if (scale > 0)
        {
...
function retina(parent)
{
    var context = document.body;
    if (parent == 1) context = window.parent.document.body;
    if ((((typeof easyedit != 'undefined') && (easyedit == 1)) || ((typeof ee != 'undefined') && (ee == 1))) && (window.devicePixelRatio > 1))
    {
        jQuery('.handle img, .pencil img, .editicon img', context).each(function() { if (this.src.indexOf('32') < 0) this.src = this.src.replace('.png', '32.png'); });
    }
}
...
function refinefields(name, value, column, brandlabel, inbrand, inprice, checkboxes, iscrollable, iname, iminprice, imaxprice, message)
{
    ...
    jQuery.post('/blocks/refinefields.php', { name: name, value: value, column: column, brandlabel: brandlabel, inbrand: inbrand, inprice: inprice, checkboxes: checkboxes, scrollable: iscrollable }, function(response)
    {
        ...
        if (responses[0].indexOf('class="lazyp"') >= 0) jQuery('img.lazyp').lazyload({ skip_invisible: false, threshold: threshold, load: function() { var mode = document.documentMode || 0; if (!jQuery.browser.msie || ((jQuery.browser.version >= 8 && !mode) || mode >= 8)) resizerow(1, 1, jQuery(this).closest('tr')); } });
...
function selectdelivery()
{
    if (jQuery('#delivery').length)
    {
        ...
        if (jQuery('#alternate').length)
        {
            ...
            if (jQuery('#alternate').is(':checked')) url += '&alternate=1'; else url += '&alternate=0';
        }
